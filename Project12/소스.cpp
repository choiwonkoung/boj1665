...
int get_ans(Spoly& poly) {
	sort(poly.num.begin(), poly.num.end(),greater<int>());
	int num=0, ans=0, ret=0;
	for (int i=0;i<poly.num.size();i++) {
		int pre = i>0?poly.num[i - 1]:poly.num[i];
		int now = poly.num[i];
		ret = max(ret,(pre - now - 1) * num + ans);	
		ans = (pre - now) * num + poly.p[now] + ans;
		ret = max(ans, ret);
		num = num + poly.p[now];
	}
	return ret;
